    ...
    if ($email != "" && $pass != "") {
        ...
        if (mysqli_num_rows($queryRun) > 0) {
            while ($row = mysqli_fetch_array($queryRun)) {
                $name = $row[1];
                setcookie("adminlogin", "1", time() + (86400 * 5), "/");
                setcookie("adminemail", $email, time() + (86400 * 5), "/");
                setcookie("adminpassword", $pass, time() + (86400 * 5), "/");
                setcookie("adminname", $name, time() + (86400 * 5), "/");
                header('Location: index.php');
            }
        }
    }
